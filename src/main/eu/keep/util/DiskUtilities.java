...
public class DiskUtilities {
	...
	public static List<Integer> determineCHS(File diskFile) throws IOException {
		...
		// Read values from boot sectors initially
		img.seek(0x18);
		...
		// Check the partition table. If this exists, re-read values from partition 1
		// Note: this is very simplistic and only reads primary partition 1  
		img.seek(0x01BE + 0x08); // Start of partition table, part. 1 first sector (LBA) 
	...
	public static boolean isISO9660(File diskFile) throws IOException {
		...
		byte[] isoSignature = new byte[]{0x01, 0x43, 0x44, 0x30, 0x30, 0x31, 0x01}; // 01 'CD001' 01
		byte[] imgData = new byte[isoSignature.length];
		RandomAccessFile img = new RandomAccessFile(diskFile, "r");
		
		if (diskFile.length() < (0x8000 + isoSignature.length))
			return false;

		img.seek(0x8000); // Sector 16 (2048 bytes/sector)
		img.read(imgData);
		if (Arrays.equals(imgData, isoSignature))
		{
			// Highly likely this is a ISO9660 optical disk
	...
	{
		if (in.length > 7)	// Cannot guarantee value will not exceed Integer.MAX_VALUE
			return -1;
